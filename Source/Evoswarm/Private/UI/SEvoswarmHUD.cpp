...
{
	// FPS is smoothed every frame in Tick() from the true per-frame delta; computing it here
	// from the ~0.1 s refresh window pinned the readout near 10 FPS regardless of reality.
...
void SEvoswarmHUD::Tick(const FGeometry& Geometry, const double CurrentTime, const float DeltaTime)
{
	...
	// FPS: exponential moving average of the true per-frame rate, sampled EVERY frame (the
	// text below only refreshes at ~10 Hz, but the rate must be measured per frame).
	if (DeltaTime > 0.f)
	{
