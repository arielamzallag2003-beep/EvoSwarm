	...
	EntityQuery.ForEachEntityChunk(Context, [](FMassExecutionContext& Context)
		{
			...
			for (FMassExecutionContext::FEntityIterator It = Context.CreateEntityIterator(); It; ++It)
			{
				...
				// Asleep: no locomotion at all. Without this the boid would keep coasting on its
				// last velocity, because zero steering force means nothing damps it.
				if (S.CurrentBehaviorState == EBoidState::Sleeping)
				{
					S.CurrentStamina = FMath::Min(Evo::MaxStamina(G),
						S.CurrentStamina + Evo::StaminaRegenPerSec * Evo::SleepStaminaRegenMult * Dt);

					Vel[It].Value = FVector::ZeroVector;
					Force[It].Value = FVector::ZeroVector;

					// Still settle onto the surface, in case the terrain moved under it.
				...
				{
					// Day/night: outside its active period a creature recovers stamina poorly, so it
					// drifts down to the sleep threshold and beds down. The population visibly beds
					// down as day flips to night -- via the existing sleep state, not a new one.
					const float Awake = Evo::Activity(Species.Nocturnality, Evo::GDaylight);
					const float RegenScale = FMath::Lerp(0.45f, 1.f, Awake);
					S.CurrentStamina = FMath::Min(Evo::MaxStamina(G),
						S.CurrentStamina + Evo::StaminaRegenPerSec * RegenScale * Dt);
				}
