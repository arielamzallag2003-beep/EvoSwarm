...
AEvoswarmPawn::AEvoswarmPawn()
{
	...
	// Movement setup kept from the merged build: the engine's hard-coded DefaultPawn_* key
	// mappings collide with the project's config (duplicate-mapping errors), so the fly axes
	// are project-owned Evo_* names bound below and mapped in DefaultInput.ini.
	bAddDefaultMovementBindings = false;
	if (UFloatingPawnMovement* Move = Cast<UFloatingPawnMovement>(GetMovementComponent()))
	{
		// ~45 m/s: the ~1 km arena stays quick to survey.
		Move->MaxSpeed = 4500.f;
...
void AEvoswarmPawn::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	{
		// Fly-camera axes: project-owned names (mapped in DefaultInput.ini), stock handlers.
		PlayerInputComponent->BindAxis(TEXT("Evo_MoveForward"), this, &ADefaultPawn::MoveForward);
		PlayerInputComponent->BindAxis(TEXT("Evo_MoveRight"), this, &ADefaultPawn::MoveRight);
		PlayerInputComponent->BindAxis(TEXT("Evo_MoveUp"), this, &ADefaultPawn::MoveUp_World);
		PlayerInputComponent->BindAxis(TEXT("Evo_Turn"), this, &ADefaultPawn::AddControllerYawInput);
		PlayerInputComponent->BindAxis(TEXT("Evo_LookUp"), this, &ADefaultPawn::AddControllerPitchInput);
		PlayerInputComponent->BindAction(TEXT("ToggleHud"), IE_Pressed, this, &AEvoswarmPawn::ToggleHud);
		...
		// Accélérateur de temps. Trois entrées pour monter/descendre parce qu'aucune n'est
		// universelle : le pavé numérique manque sur les portables, et la rangée du haut
		// dépend de la disposition du clavier.
		PlayerInputComponent->BindKey(EKeys::Add, IE_Pressed, this, &AEvoswarmPawn::SpeedUp);
		PlayerInputComponent->BindKey(EKeys::Equals, IE_Pressed, this, &AEvoswarmPawn::SpeedUp);
		PlayerInputComponent->BindKey(EKeys::MouseScrollUp, IE_Pressed, this, &AEvoswarmPawn::SpeedUp);
		PlayerInputComponent->BindKey(EKeys::Subtract, IE_Pressed, this, &AEvoswarmPawn::SpeedDown);
		PlayerInputComponent->BindKey(EKeys::Hyphen, IE_Pressed, this, &AEvoswarmPawn::SpeedDown);
		PlayerInputComponent->BindKey(EKeys::MouseScrollDown, IE_Pressed, this, &AEvoswarmPawn::SpeedDown);
		PlayerInputComponent->BindKey(EKeys::P, IE_Pressed, this, &AEvoswarmPawn::TogglePauseSim);
	}
}

void AEvoswarmPawn::SpeedUp()
{
...
void AEvoswarmPawn::TogglePauseSim()
{
	if (UEvoswarmSimSubsystem* Sim = GetSim())
	{
		Sim->TogglePause();
...
void AEvoswarmPawn::ToggleHud()
{
	if (APlayerController* PC = Cast<APlayerController>(GetController()))
	{
		if (AEvoswarmHUD* Hud = Cast<AEvoswarmHUD>(PC->GetHUD()))
		{
			Hud->TogglePanel();
		}
	}
...
void AEvoswarmPawn::ToggleDebug()
{
	if (UWorld* World = GetWorld())
	{
		...
		{
			Sim->ToggleDebugDraw();
			// Turning the overlay on while the mode is 0 would show almost nothing, which reads
			// as "B is broken". Land on the behaviour-state overlay, the most useful default.
			if (Sim->IsDebugDraw() && Sim->GetDebugMode() == 0)
			{
				Sim->SetDebugMode(2);
			}
		}
...
void AEvoswarmPawn::ApplyDebugMode(int32 Mode)
{
	if (UWorld* World = GetWorld())
	{
		if (UEvoswarmSimSubsystem* Sim = World->GetSubsystem<UEvoswarmSimSubsystem>())
		{
			...
			// A numpad key now implies the overlay state, so 1-4 work on their own (as in the
			// reference build) instead of silently doing nothing until B was pressed:
			//   1-4 -> select the overlay AND switch it on
			//   0   -> switch it off entirely
			const bool bWantOn = (Mode > 0);
			if (Sim->IsDebugDraw() != bWantOn)
			{
				Sim->ToggleDebugDraw();
			}
		}
