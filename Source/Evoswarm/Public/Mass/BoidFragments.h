...
/**
 * Ce que la créature poursuit actuellement. Le steering connaît déjà cette cible : il la
 * mémorise ici pour que l'overlay de debug puisse tracer la flèche SANS relancer une
 * requête spatiale par créature et par frame (c'est ce coût-là qui figeait l'éditeur).
 */
UENUM()
enum class EBoidDebugTarget : uint8
{
	None,
	Plant,    // plante visée (herbivore en quête)
	Carcass,  // charogne visée (charognard)
	Prey,     // proie vivante traquée
	Mate      // partenaire courtisé
};
...
struct EVOSWARM_API FBoidStateFragment : public FMassFragment
{
	...
	/** Last steering force computed. Re-applied on the frames this boid skips under the
	 *  camera-distance LOD (see Evo::SteerStride), so distant boids keep moving smoothly. */
	FVector CachedSteer = FVector::ZeroVector;

	/** Cible poursuivie, mémorisée par le steering pour l'overlay de debug (mode 2). */
	FVector DebugTargetPos = FVector::ZeroVector;
	EBoidDebugTarget DebugTargetKind = EBoidDebugTarget::None;
};
...
struct EVOSWARM_API FBoidSpeciesSharedFragment : public FMassSharedFragment
{
	...
	/** 0 = diurnal (awake by day), 1 = nocturnal (awake by night). Drives the day/night
	 *  activity cycle: perception and stamina recovery key off it via Evo::Activity(). */
	UPROPERTY()
