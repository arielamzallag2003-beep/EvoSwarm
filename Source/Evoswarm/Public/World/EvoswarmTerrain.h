...
namespace Evo
{
	...
	inline constexpr float BiomeFreq = 1.f / 9000.f;       // big organic biome patches
	...
	inline float Ruggedness(float X, float Y) { return Fbm01(X, Y, BiomeFreq, 2, 47.3f); }
	/** Independent moisture field [0,1]. */
	inline float Moisture(float X, float Y)   { return Fbm01(X, Y, BiomeFreq, 2, 28.6f); }
	...
	/** Continuous ground height at a world XY (cm). */
