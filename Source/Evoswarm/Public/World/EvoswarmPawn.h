...
// - + / - :      accélérateur de temps (pavé numérique, rangée du haut, ou molette)
// - P key:       pause / reprise de la simulation (la caméra reste libre)
...
private:
	void ToggleDebug();
	void ToggleHud();   // H: show/hide the stats panel (kept from the merged build)
	void ToggleSelect();
	...
	// Accélérateur de temps : + / - (pavé numérique ou rangée du haut) et molette souris
	// montent et descendent d'un cran ; P met en pause sans figer la caméra.
	void SpeedUp();
	void SpeedDown();
	void TogglePauseSim();
